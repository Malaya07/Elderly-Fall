...
while i < 100:
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        score_list.append(precision_score(
            y_test, y_pred, labels=[0, 1], average='weighted'))
        score_list.append(recall_score(
            y_test, y_pred, labels=[0, 1], average='weighted'))
        score_list.append(recall_score(
            y_test, y_pred, labels=[0, 1], average='weighted'))
        score_list.append(
            f1_score(y_test, y_pred, average='weighted', labels=[0, 1]))
